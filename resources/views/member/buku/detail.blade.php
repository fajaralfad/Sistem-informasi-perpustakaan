            ...
            <div class="md:w-2/3 lg:w-3/4 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <button type="button" 
                        ...
                        onclick="toggleWishlist({{ $buku->id }})">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path>
                    </svg>
                    <span id="wishlistText">Tambah ke Favorit</span>
                </button>
                </div>
...
function checkWishlistStatus(bukuId) {
    fetch(`/member/wishlist/check/${bukuId}`)
        ...
        .then(data => {
            ...
            if (data.exists) {
                button.classList.add('bg-green-100', 'dark:bg-green-900', 'text-green-800', 'dark:text-green-200');
                button.classList.remove('bg-gray-100', 'dark:bg-gray-700', 'hover:bg-gray-200', 'dark:hover:bg-gray-600');
                text.textContent = 'Sudah di Favorit';
                button.onclick = null;
...
function toggleWishlist(bukuId) {
    const button = document.getElementById('wishlistButton');
    const text = document.getElementById('wishlistText');
    
    // Tampilkan loading state
    button.innerHTML = `
        <svg class="animate-spin -ml-1 mr-2 h-5 w-5 text-gray-800 dark:text-gray-200" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    fetch("{{ route('member.wishlist.store') }}", {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            ...
            button.innerHTML = `
                <svg class="w-5 h-5 mr-2 text-green-600 dark:text-green-300" fill="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                </svg>
                <span id="wishlistText">Sudah di Favorit</span>
            `;
            button.classList.add('bg-green-100', 'dark:bg-green-900', 'text-green-800', 'dark:text-green-200');
            button.classList.remove('bg-gray-100', 'dark:bg-gray-700', 'hover:bg-gray-200', 'dark:hover:bg-gray-600');
            button.disabled = true;
            ...
            // Refresh daftar wishlist jika ada di dashboard
            if (typeof refreshWishlist === 'function') {
                refreshWishlist();
            }
        } else {
            showErrorToast(data.message || 'Gagal menyimpan ke favorit');
            resetWishlistButton();
        }
    })
    .catch(error => {
        console.error('Error:', error);
        showErrorToast('Buku sudah ada di daftar favorit Anda');
        resetWishlistButton();
    });
}

// Fungsi untuk reset button ke state semula
...
function showSuccessToast(message) {
    ...
    toast.className = 'fixed bottom-4 right-4 bg-green-500 text-white px-4 py-2 rounded-lg shadow-lg flex items-center animate-fade-in';
    toast.innerHTML = `
        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
        </svg>
...
function showErrorToast(message) {
    const toast = document.createElement('div');
    toast.className = 'fixed bottom-4 right-4 bg-red-500 text-white px-4 py-2 rounded-lg shadow-lg flex items-center animate-fade-in';
    toast.innerHTML = `
        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
