...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8">
        ...
        <!-- Summary Cards -->
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6 lg:mt-0">
            <div class="bg-red-50 border border-red-200 rounded-lg p-4 text-center">
                <div class="text-2xl font-bold text-red-700">Rp {{ number_format($totalDendaBelumLunas, 0, ',', '.') }}</div>
                <div class="text-sm text-red-600">Belum Lunas</div>
            </div>
            <div class="bg-green-50 border border-green-200 rounded-lg p-4 text-center">
                <div class="text-2xl font-bold text-green-700">Rp {{ number_format($totalDendaLunas, 0, ',', '.') }}</div>
                <div class="text-sm text-green-600">Sudah Lunas</div>
            </div>
        </div>
    </div>
            ...
            <div class="flex-1 lg:max-w-md">
                ...
                <div class="relative">
                    ...
                    <input type="text" id="search" name="search" 
                           ...
                           placeholder="Cari judul buku atau keterangan..." onkeyup="searchTable()">
                </div>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    <div class="text-sm font-medium text-gray-900">{{ Str::limit($denda->peminjaman->buku->judul, 40) }}</div>
                        ...
                        <td class="px-6 py-4">
                            <div class="text-sm text-gray-900">{{ $denda->keterangan }}</div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            <div>{{ $denda->created_at->format('d M Y') }}</div>
                            @if($denda->tanggal_bayar)
                            <div class="text-xs text-green-600">Dibayar: {{ $denda->tanggal_bayar->format('d M Y') }}</div>
                            @endif
                        </td>
                            ...
                            <div class="flex justify-center space-x-2">
                                ...
                                @if(!$denda->status_pembayaran)
                                    @if($denda->bukti_pembayaran && !$denda->is_verified)
                                        ...
                                        <form action="{{ route('member.denda.batalkan', $denda->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin membatalkan pembayaran ini?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" 
                                                    class="bg-gray-100 hover:bg-gray-200 text-gray-800 px-3 py-2 rounded-lg text-xs font-medium inline-flex items-center transition-colors duration-200"
        ...
        <div class="text-center py-12">
            ...
            <h3 class="mt-2 text-sm font-medium text-gray-900">Tidak ada denda</h3>
            <p class="mt-1 text-sm text-gray-500">Selamat! Anda tidak memiliki denda perpustakaan.</p>
        </div>
        @endif
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    const alerts = document.querySelectorAll('.alert-error, .alert-success, .alert-warning, .alert-info');
