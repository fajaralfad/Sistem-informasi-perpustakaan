...
<div class="min-h-screen bg-gray-900">
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-gray-800 shadow-lg rounded-xl overflow-hidden border border-gray-700">
            <form action="{{ isset($buku) ? route('admin.buku.update', $buku->id) : route('admin.buku.store') }}" 
                    ...
                    <div class="space-y-4">
                            ...
                            <select name="kategori_id" id="kategori_id" 
                                ...
                                @foreach($kategoris as $kategori)
                                    <option value="{{ $kategori->id }}" 
                                            {{ (old('kategori_id') ?? (isset($buku) ? $buku->kategori_id : '')) == $kategori->id ? 'selected' : '' }}>
                                        {{ $kategori->nama }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="pengarang_id" id="pengarang_id" 
                                ...
                                @foreach($pengarangs as $pengarang)
                                    <option value="{{ $pengarang->id }}" 
                                            {{ (old('pengarang_id') ?? (isset($buku) ? $buku->pengarang_id : '')) == $pengarang->id ? 'selected' : '' }}>
                                        {{ $pengarang->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            <label for="cover" class="block text-sm font-medium text-gray-300 mb-1">
                                Cover Buku {{ isset($buku) ? '(kosongkan jika tidak ingin mengubah)' : '' }}
                            </label>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="stok" id="stok" min="1" 
                                   value="{{ old('stok', isset($buku) ? $buku->stok : 1) }}"
                                   class="block w-full bg-gray-700 border border-gray-600 rounded-lg shadow-sm py-2 px-3 text-gray-600 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out"
...
const isEditMode = {{ isset($buku) ? 'true' : 'false' }};
const existingIsbns = @json(isset($buku) && $buku->isbn ? (is_array($buku->isbn) ? $buku->isbn : json_decode($buku->isbn, true)) : []);
...
function generateIsbnFields() {
    ...
    for (let i = 1; i <= stok; i++) {
        ...
        const oldValue = `{{ old('isbn.${i-1}', '') }}`;
        const fieldValue = oldValue || existingValue;
